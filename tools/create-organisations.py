...
def main(c_args: argparse.Namespace, filename: str) -> None:
    ...
    if not org_rv.success:
        ...
        sys.exit(1)
    existing_org_names: List[str] = [org['name'] for org in org_rv.msg['organisations']]
    ...
    num_orgs: int = 0
    num_orgs_failed: int = 0
    num_orgs_skipped: int = 0
    for org in orgs:
        ...
        if name in existing_org_names:
            console.log(f':white_check_mark: Skipping "{name}" - it already exists')
            num_orgs_skipped += 1
        else:
            org_rv: AsApiRv = AsApi.create_organisation(token, org_name=name, org_owner=owner)
            if org_rv.success:
                num_orgs += 1
                ...
                emoji = ':cross_mark:'
            # Log
            console.log(f'{emoji} {name} ({owner})')

    # Summary
    if num_orgs:
        console.log(f'Organisations created {num_orgs}')
    if num_orgs_skipped:
        console.log(f'Organisations skipped {num_orgs_skipped}')
    # Error states
    if num_orgs_failed:
        console.log(f'Organisations failed {num_orgs_failed}')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Creates Organisations (from a YAML file). You will need admin privileges to use this tool."
    )
